...
void init_idt(void)
{
    ...
    init_idt_entry(&vectors[0x80] ,(uint64_t)syscall, 0xee);    //epl set to ring 3
...
void handler(struct TrapFrame *tf)
{
    ...
    switch (tf->trapno) {
        ...
        case 0x80:
            ...
            break;
        default:
            printk("Error %d at ring %d with code %d at rip 0x%x addr 0x%x\n",
            tf->trapno, (tf->cs & 3), tf->errorcode, tf->rip, read_cr2());
            while (1) { }
    }
}